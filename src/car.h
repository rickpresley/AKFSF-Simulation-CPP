...
public:
    ...
    MotionCommandTurnTo(double command_heading, double command_velocity) :
        m_command_heading(command_heading),
        m_command_velocity(command_velocity)
    {
    }
...
public:
    ...
    MotionCommandMoveTo(double command_x, double command_y, double command_velocity) :
        m_command_x(command_x),
        m_command_y(command_y),
        m_command_velocity(command_velocity)
    {
    }
...
public:
    ...
    BicycleMotion() :
        m_initial_state(VehicleState(0, 0, 0, 0)),
        m_wheel_base(4.0),
        m_max_velocity(28.0),
...
public:
    ...
    void render(Display& disp)
    {
        ...
        disp.drawLines(transformPoints(m_car_lines_body, carPosOffset,
            carPsiOffset));
        disp.drawLines(transformPoints(m_marker_lines, carPosOffset,
            carPsiOffset));

        disp.setDrawColour(0, 201, 0);
        disp.drawLines(transformPoints(transformPoints(m_wheel_lines,
            m_wheel_fl_offset, steeringPsi), carPosOffset, carPsiOffset));
        disp.drawLines(transformPoints(transformPoints(m_wheel_lines,
            m_wheel_fr_offset, steeringPsi), carPosOffset, carPsiOffset));
        disp.drawLines(transformPoints(offsetPoints(m_wheel_lines,
            m_wheel_rl_offset), carPosOffset, carPsiOffset));
        disp.drawLines(transformPoints(offsetPoints(m_wheel_lines,
            m_wheel_rr_offset), carPosOffset, carPsiOffset));
    }
